...
function autoPrepareDeal(room) {
  if (room.status !== "dealer_prepare") return;
  room.events.unshift("庄家洗牌倒计时结束，系统默认不洗牌发牌。");
  prepareDeal(room, dealer(room), false);
}
...
function applyTimeouts(room) {
  ...
  if (room.status === "dealer_prepare" && room.turnDeadlineAt && Date.now() >= room.turnDeadlineAt) {
    autoPrepareDeal(room);
  }
  let guard = 0;
  while (
